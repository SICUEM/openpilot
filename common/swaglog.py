...
def send_logs(data):
    ...
    try:
        p.produce(logs_topic, value=data.encode('utf-8'))
        p.flush()
        print("Logs enviados correctamente")
    except Exception as e:
        print(f"Error al enviar logs: {e}")

# Función para enviar datos de cámaras a Kafka
def send_camera_data(data):
    p = Producer({
        'bootstrap.servers': bootstrap_servers,
        'api.version.request': 'true',
    })
    try:
        p.produce(camera_topic, value=data.encode('utf-8'))
        p.flush()
        print("Datos de cámara enviados correctamente")
    except Exception as e:
        print(f"Error al enviar datos de cámara: {e}")

# Función para obtener el manejador de archivos de log
    ...
    def doRollover(self):
        ...
        try:
            with open(f"{self.base_filename}.{self.last_file_idx:010}", 'r') as file:
                data = file.read()
            send_logs(data)
            send_camera_data(data)  # Envía los datos de la cámara
