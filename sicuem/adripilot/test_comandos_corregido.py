...
# -*- coding: utf-8 -*-
"""
Script de prueba para verificar que el sistema MQTT funciona correctamente
...
def send_control_forward():
    """Envía comando de control forward."""
    topic = f"telemetry_config/{DONGLE_ID}/control"
    message = '{"forward": true}'
    print(f"🚀 ===== COMANDO CONTROL FORWARD ===== 🚀")
    print(f"📱 Dispositivo: {DONGLE_ID}")
    print(f"📡 Topic MQTT: {topic}")
    print(f"📤 Mensaje: {message}")
    print(f"🌐 Broker: {BROKER_ADDRESS}:{BROKER_PORT}")
...
def main():
    print("🚀 Prueba de Comandos del Servidor AdriPilot - CORREGIDO")
    print("📋 Usando el formato real que envía el servidor")
    ...
    try:
        ...
        print("\n--- PRUEBA 3: Comando Control Forward ---")
        print("🎯 Enviando: telemetry_config/UnregisteredDevice/control -> {\"forward\": true}")
        send_control_forward()
        time.sleep(3)

        print("\n--- PRUEBA 4: Múltiples comandos ---")
        for i in range(2):
            print(f"\n   Comando {i+1}/2 - Disminuir")
            send_speed_down()
            time.sleep(2)

            print(f"   Comando {i+1}/2 - Aumentar")
            send_speed_up()
            time.sleep(2)
        ...
        print("   - 🚀 Comando FORWARD activado")
