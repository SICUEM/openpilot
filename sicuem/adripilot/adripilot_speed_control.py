...
Modifica directamente múltiples variables de velocidad
"""
import time
from openpilot.common.params import Params

class AdriPilotSpeedControl:
  """Controlador de velocidad directo para AdriPilot."""
  ...
  def process_speed_commands(self, car_control, car_state, controls_state):
    ...
    if speed_increase or speed_decrease:
      # Obtener velocidad actual desde múltiples fuentes
      current_speed = self.get_current_speed(car_control, controls_state)
      ...
      # Aplicar cambios a múltiples variables
  ...
  def get_current_speed(self, car_control, controls_state):
    """Obtiene la velocidad actual desde múltiples fuentes."""
    # Prioridad 1: setSpeed en hudControl (m/s)
    ...
    # Prioridad 2: vCruise en controlsState
    if hasattr(controls_state, 'vCruise') and controls_state.vCruise > 0:
      return float(controls_state.vCruise)

    # Prioridad 3: vCruise en car_control
    if hasattr(car_control, 'vCruise') and car_control.vCruise > 0:
      return float(car_control.vCruise)

    # Fallback: 40 km/h
    return 40.0

  def apply_speed_changes(self, car_control, new_speed_kmh):
    """Aplica cambios de velocidad a múltiples variables."""
...
adripilot_speed_control = AdriPilotSpeedControl()
