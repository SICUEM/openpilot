...
class CustomRouteManager:
  def __init__(self, sm, pm, waypoints):
    self.sm = sm  # SubMaster para recibir datos del vehículo
    self.pm = pm  # PubMaster para enviar datos al vehículo
    self.waypoints = [Coordinate(*wp) for wp in waypoints]  # Lista de waypoints convertidos a objetos Coordinate
    self.current_waypoint_idx = 0  # Índice del waypoint actual

    self.params = Params()  # Parámetros del sistema
    self.last_position = coordinate_from_param("LastGPSPosition", self.params)  # Última posición GPS conocida
    self.last_bearing = None  # Última dirección de movimiento conocida

    self.gps_ok = False  # Estado del GPS
    self.localizer_valid = False  # Validez del localizador

    self.nav_destination = None  # Destino de navegación actual
    self.step_idx = None  # Índice del paso actual en la ruta
    self.route = None  # Ruta actual
    self.route_geometry = None  # Geometría de la ruta actual

    self.recompute_backoff = 0  # Retraso exponencial para la recomputación
    self.recompute_countdown = 0  # Contador de recomputación

    self.api = None  # API para obtener el token de Mapbox
    self.mapbox_token = None  # Token de Mapbox
  ...
  def update_location(self):
    # Actualiza la ubicación y estado del GPS
    location = self.sm['liveLocationKalman']
    self.gps_ok = location.gpsOK
    self.localizer_valid = (location.status == log.LiveLocationKalman.Status.valid) and location.positionGeodetic.valid

    if self.localizer_valid:
      # Actualiza la última posición y dirección si el localizador es válido
      self.last_bearing = math.degrees(location.calibratedOrientationNED.value[2])
      self.last_position = Coordinate(location.positionGeodetic.value[0], location.positionGeodetic.value[1])

  def recompute_route(self):
    # Recalcula la ruta hacia el siguiente waypoint
    if self.last_position is None:
      return

    if self.current_waypoint_idx >= len(self.waypoints):
      # Todos los waypoints han sido alcanzados
      cloudlog.warning("All waypoints reached")
      self.clear_route()
      return

    new_destination = self.waypoints[self.current_waypoint_idx]  # Próximo destino
    should_recompute = self.should_recompute()
    if new_destination != self.nav_destination:
      # Se ha recibido un nuevo destino
      cloudlog.warning(f"Got new destination from waypoints list {new_destination}")
